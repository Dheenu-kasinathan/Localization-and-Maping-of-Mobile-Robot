...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        x_1 = old_pose[0] - scanner_displacement*cos(old_pose[2])
        y_1 = old_pose[1] - scanner_displacement*sin(old_pose[2])
        
        alpha = ((motor_ticks[1]*ticks_to_mm)-(motor_ticks[0]*ticks_to_mm))/robot_width
        
        R = (motor_ticks[0]*ticks_to_mm)/alpha
        cx = x_1 - ((R+(robot_width/2))*(sin(old_pose[2])))
        cy = y_1 - ((R+(robot_width/2))*(-cos(old_pose[2])))
